<div>
    ...
    <div class="mb-8 flex items-center justify-between">
        <div>
            ...
            <h1 class="mt-1 font-serif text-3xl text-[#e8dfca]">Regions</h1>
        </div>
    ...
    <div class="mb-8 grid grid-cols-2 border border-[#2c2922] bg-[#151310] sm:grid-cols-3">
        <div class="border-b border-r border-[#2c2922] p-5 sm:border-b-0">
            <p class="text-[9px] uppercase tracking-[0.25em] text-[#806337]">Regions Shown</p>
        ...
        <div class="p-5">
            <p class="text-[9px] uppercase tracking-[0.25em] text-[#806337]">Most Populous</p>
    ...
    <div class="mb-6 border border-[#2c2922] bg-[#151310] p-5">
        ...
        @if ($search || $minKingdoms > 0 || $maxKingdoms < 100)
                        ...
                        <tr class="transition hover:bg-[#191611]">
                            ...
                            <td class="px-5 py-4">
                                <div class="flex items-center justify-end gap-4 text-[9px] uppercase tracking-[0.15em]">
                                    <button wire:click="openEdit('{{ $region->slug }}')"
                                        class="text-[#a17e43] transition hover:text-[#c59b4a]">Edit</button>
                                    <button wire:click="openDelete('{{ $region->slug }}')"
                    ...
                    @empty
                        <tr>
                            <td colspan="4" class="px-5 py-16 text-center">
                                @if ($search || $minKingdoms > 0 || $maxKingdoms < 100)
                                    <p class="font-serif text-sm text-[#8f826b]">No regions match these filters.</p>
                                    <p class="mt-1 text-xs text-[#625744]">
                                        <button wire:click="clearFilters" class="underline hover:text-[#c8b895]">Clear filters</button>
                                        to see the full map.
                                    </p>
                                @else
                                    <p class="font-serif text-sm text-[#8f826b]">No regions charted.</p>
                                    <p class="mt-1 text-xs text-[#625744]">Begin by mapping the first territory.</p>
                                @endif
                            </td>
                        </tr>
                    @endforelse
    ...
    <div class="mt-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <p class="text-[9px] uppercase tracking-[0.15em] text-[#625744]">
            Showing {{ $regions->count() }} of {{ $summary['total'] }}
        </p>

        <div class="flex w-full gap-2 sm:w-auto">
            <button wire:click="goToCursor('{{ $regions->previousCursor()?->encode() }}')" @if (!$regions->previousCursor()) disabled @endif
            ...
            <button wire:click="goToCursor('{{ $regions->nextCursor()?->encode() }}')" @if (!$regions->hasMorePages()) disabled @endif
                class="flex-1 border border-[#3b3225] px-4 py-2 text-[9px] uppercase tracking-[0.15em] text-[#857861] transition hover:border-[#806337]/40 hover:text-[#c8b895] disabled:opacity-30 sm:flex-none">
                Next →
            </button>
        </div>
